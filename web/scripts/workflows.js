...
import {
  ComfyAsyncDialog
} from "./ui/components/asyncDialog.js";
import {
  getStorageValue,
  setStorageValue
} from "./utils.js";
...
export class ComfyWorkflowManager extends EventTarget {
  ...
  #bindExecutionEvents() {
    ...
    const emit = () => this.dispatchEvent(new CustomEvent("execute", {
      detail: this.activePrompt
    }));
    let executing = null;
    api.addEventListener("execution_start", (e) => {
      ...
      this.queuedPrompts[this.#activePromptId] ??= {
        nodes: {}
      };
      emit();
    });
...
export class ComfyWorkflow {
  ...
  async favorite(value) {
    try {
      ...
      this.manager.dispatchEvent(new CustomEvent("favorite", {
        detail: this
      }));
    } catch (error) {
  ...
  async rename(path) {
    ...
    if (resp.status === 409) {
      ...
      resp = await api.moveUserData("workflows/" + this.path, "workflows/" + path, {
        overwrite: true
      });
    }
    ...
    this.manager.dispatchEvent(new CustomEvent("rename", {
      detail: this
    }));
  ...
  async delete() {
    ...
    try {
      ...
      this.manager.dispatchEvent(new CustomEvent("delete", {
        detail: this
      }));
    } catch (error) {
  ...
  async #save(path, overwrite) {
    ...
    if (!this.path) {
      ...
      // Normal save
      this.unsaved = false;
      this.manager.dispatchEvent(new CustomEvent("save", {
        detail: this
      }));
    }
