...
import {
  createImageHost,
  calculateImageGrid
} from "./ui/imagePreview.js";
import {
  ComfyAppMenu
} from "./ui/menu/index.js";
import {
  getStorageValue,
  setStorageValue
} from "./utils.js";
...
export class ComfyApp {
          ...
          if (imageIndex == null) {
            ...
            if (!compact_mode) {
              ...
              ({
                ...
                cols,
                shiftX
              } = calculateImageGrid(this.imgs, dw, dh));
            }
  ...
  async #loadExtensions() {
    ...
    this.logging.addEntry("Comfy.App", "debug", {
  ...
  async #setUser() {
    ...
    if (!user || !users[user]) {
      ...
      const {
        UserSelectionScreen
      } = await import("./ui/userSelection.js");
  ...
  async queuePrompt(number, batchCount = 1) {
    ...
    api.dispatchEvent(new CustomEvent("promptQueued", {
      detail: {
        number,
        batchCount
      }
  ...
  showErrorOnFileLoad(file) {
    this.ui.dialog.show(
      $el("div", [
        $el("p", {
          textContent: `Unable to find workflow in ${file.name}`
        })
      ]).outerHTML
    );
  }
