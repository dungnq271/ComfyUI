...
function createIntWidget(node, inputName, inputData, app, isSeedInput) {
  ...
  const {
    val,
    config
  } = getNumberDefaults(inputData, 1, 0, true);
...
export const ComfyWidgets = {
  ...
  FLOAT(node, inputName, inputData, app) {
    ...
    const {
      val,
      config
    } = getNumberDefaults(inputData, 0.5, precision, !disable_rounding);
    return {
      widget: node.addWidget(widgetType, inputName, val,
        function(v) {
          if (config.round) {
  ...
  STRING(node, inputName, inputData, app) {
    ...
    let res;
    if (multiline) {
      res = addMultilineWidget(node, inputName, {
        defaultVal,
        ...inputData[1]
      }, app);
    } else {
