...
export class GroupNodeConfig {
      ...
      if (widgetType) {
        ...
        if (convertedIndex > -1) {
          ...
          const {
            name,
            config
          } = this.getInputConfig(node, inputName, seenInputs, inputs[inputName]);
  ...
  processNodeInputs(node, seenInputs, inputs) {
    ...
    const {
      converted,
      slots
    } = this.processWidgetInputs(inputs, node, inputNames, seenInputs);
    const linksTo = this.linksTo[node.index] ?? {};
    ...
    function handleEvent(type, getId, getEvent) {
      const handler = ({
        ...
        if (innerNodeIndex > -1) {
          ...
          api.dispatchEvent(new CustomEvent(type, {
