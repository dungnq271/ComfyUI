...
class FourierFeatures(nn.Module):
    ...
    def forward(self, input):
        f = (
            ...
            * math.pi
            * input
            @ self.weight.T.to(dtype=input.dtype, device=input.device)
        )
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        return F.layer_norm(
            x,
            x.shape[-1:],
            weight=self.gamma.to(dtype=x.dtype, device=x.device),
            bias=beta,
        )
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        freqs = torch.einsum(
            "i , j -> i j", t, self.inv_freq.to(dtype=dtype, device=device)
        )
        ...
        scale = self.scale.to(dtype=dtype, device=device) ** rearrange(
            power, "n -> n 1"
        )
