    ...
    def __init__(self, model_dim, ff_dim, ff_activation, dtype, device, operations):
        ...
        if ff_activation == "gelu_pytorch_tanh":
            self.DenseReluDense = T5DenseGatedActDense(
                model_dim, ff_dim, dtype, device, operations
            )
...
class T5Block(torch.nn.Module):
    def __init__(
        ...
        self.layer.append(
            T5LayerFF(model_dim, ff_dim, ff_activation, dtype, device, operations)
        )
...
class T5(torch.nn.Module):
    def __init__(self, config_dict, dtype, device, operations):
        ...
        self.dtype = dtype
        self.shared = torch.nn.Embedding(
            config_dict["vocab_size"], model_dim, device=device
        )
